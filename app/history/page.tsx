...
export default function HistoryPage() {
    ...
    const handleCancel = async (id: number) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                toast.error(errData.message || 'Gagal membatalkan reservasi.');
            }
        } catch (error: any) {
            toast.error('Terjadi kesalahan server saat membatalkan.');
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleOpenEdit = (res: Reservation) => {
        ...
        const rejection = res.rejection_reason || res.rejectionReason || '';
        setEditRejectionReason(rejection);
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const errData = await res.json();
                toast.error(errData.message || 'Gagal memperbarui status reservasi.');
            }
        } catch (error: any) {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const errData = await res.json();
                toast.error(errData.message || 'Gagal menghapus riwayat.');
            }
        } catch (error: any) {
    ...
    // Helper functions for dates & driver
    const getStartDate = (res: Reservation) => res.start_date || res.startDate || '';
    const getEndDate = (res: Reservation) => res.end_date || res.endDate || '';
    const getDriverName = (res: Reservation) => res.driver_name || res.driverName || '';
    const getRejectionReason = (res: Reservation) => res.rejection_reason || res.rejectionReason || '';
    const getCreatedAt = (res: Reservation) => res.created_at || res.createdAt || getStartDate(res);
    ...
    const columns = [
            ...
            render: (res: Reservation) => (
                <div className="flex flex-col space-y-1 min-w-[90px]">
                    ...
                    <span className="text-[10.5px] text-slate-400 font-medium">
                        {getCreatedAt(res) ? new Date(getCreatedAt(res)).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' }) : '-'}
                    </span>
                </div>
            )
        },
        ...
        },
        {
            ...
            render: (res: Reservation) => {
                const sDate = getStartDate(res);
                const eDate = getEndDate(res);
                return (
        ...
        },
        {
            ...
            render: (res: Reservation) => {
                const driver = getDriverName(res);
                return (
                    <div className="flex flex-col space-y-1.5 min-w-[200px] max-w-[260px]">
                        ...
                        {driver && (
                            <span className="inline-flex items-center gap-1.5 text-[10.5px] font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/50 px-2.5 py-0.5 rounded-lg border border-emerald-200/60 dark:border-emerald-900/40 w-fit">
                                <User className="w-3 h-3 text-emerald-600" /> Driver: {driver}
                            </span>
                        )}
                    </div>
                );
        ...
        },
        {
            ...
            header: 'Status',
            render: (res: Reservation) => {
                const rejection = getRejectionReason(res);
                return (
                    <div className="flex flex-col space-y-1 min-w-[120px]">
                        <Badge status={res.status} />
                        {res.status === 'rejected' && rejection && (
                            <span className="text-[10.5px] text-red-500 font-medium max-w-[150px] line-clamp-2" title={rejection}>
                                Alasan: {rejection}
                            </span>
                        )}
                    </div>
                    ...
                    <div className="space-y-5 py-1 font-sans">
                        ...
                        <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200/60 dark:border-slate-700/60">
                            ...
                            <span className="text-xs text-slate-400 font-medium">
                                Diajukan: {getCreatedAt(selectedRes) ? new Date(getCreatedAt(selectedRes)).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : '-'}
                            </span>
                        </div>
                        ...
                        <div className="p-4 bg-slate-50/50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-800 space-y-3 text-xs">
                            ...
                            <div className="pl-5 pt-2 border-t border-slate-200/60 dark:border-slate-700/60 flex items-center justify-between">
                                ...
                                <span className="font-semibold text-slate-800 dark:text-white">
                                    {(selectedRes.driver_required || selectedRes.driverRequired) ? 'Dibutuhkan Driver' : 'Tanpa Driver'}
                                </span>
                            </div>

                            {getDriverName(selectedRes) && (
                                <div className="pl-5 flex items-center justify-between bg-emerald-50 dark:bg-emerald-950/30 p-2.5 rounded-lg border border-emerald-100 dark:border-emerald-900/30">
                                    ...
                                    <span className="font-extrabold text-emerald-800 dark:text-emerald-300">{getDriverName(selectedRes)}</span>
                                </div>
                            )}
                        </div>
                        ...
                        {getRejectionReason(selectedRes) && (
                            <div className="p-4 bg-red-50 dark:bg-red-950/40 rounded-xl border border-red-100 dark:border-red-900/30 space-y-1 text-xs text-red-700 dark:text-red-300">
                                ...
                                <p className="pl-6 font-medium">{getRejectionReason(selectedRes)}</p>
                            </div>
                        )}
                ...
                {selectedRes && (
                    <div className="space-y-4 py-1 font-sans text-xs">
                        ...
                        <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200/60 dark:border-slate-700/60 flex flex-col space-y-1">
                            ...
                            <span className="text-slate-500 dark:text-slate-400">Pemohon: <strong className="text-slate-700 dark:text-slate-200">{selectedRes.user?.name}</strong> ({selectedRes.user?.division?.name || 'Divisi'})</span>
                        </div>
